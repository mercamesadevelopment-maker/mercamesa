...
import { 
  ...
  Clock, MapPin, CreditCard, Image as ImageIcon 
} from 'lucide-react';
import { OrderDetail, OrderStatus, ORDER_STATUS_LABELS } from '../types/order.types';
...
export function OrderCard({ order, onRate }: OrderCardProps) {
  ...
  const getStatusInfo = (status: OrderStatus | null) => {
    const label = status ? ORDER_STATUS_LABELS[status] : 'Desconocido';
    
    switch (status) {
      case 'pending': return { label, color: 'bg-warnl text-warn', icon: Clock };
      case 'confirmed': return { label, color: 'bg-bluel text-blue', icon: CheckCircle2 };
      case 'paid': return { label, color: 'bg-okl text-ok', icon: CreditCard };
      case 'packing': return { label, color: 'bg-bluel text-blue', icon: Package };
      case 'at_collection': return { label, color: 'bg-purple-100 text-purple-600', icon: MapPin };
      case 'dispatched': return { label, color: 'bg-purple-100 text-purple-600', icon: Truck };
      case 'delivered': return { label, color: 'bg-okl text-ok', icon: CheckCircle2 };
      case 'cancelled': return { label, color: 'bg-rl text-r', icon: XCircle };
      case 'returned': return { label, color: 'bg-mm-gbg text-mm-txw', icon: XCircle };
      default: return { label: label || 'Desconocido', color: 'bg-mm-gbg text-mm-txw', icon: Clock };
    }
  };
    ...
    >
      <div className="p-6 border-b border-mm-crd flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="font-bold text-mm-g">{order.store_name}</h3>
            <p className="text-xs text-mm-txw">
              Pedido #{order.order_id?.slice(0, 8)} • {order.created_at ? new Date(order.created_at).toLocaleDateString() : ''}
            </p>
          </div>
        </div>
              ...
              <div key={idx} className="flex items-center justify-between text-sm">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 bg-mm-gbg rounded-lg flex items-center justify-center text-lg overflow-hidden shrink-0 border border-mm-crd/50">
                    {item.image_url ? (
                      <img src={item.image_url} alt={item.catalog_name} className="w-full h-full object-cover" />
                    ) : (
                      <ImageIcon className="w-4 h-4 text-mm-txw" />
                    )}
                  </div>
                  <span className="text-mm-txs font-medium">{item.quantity}x {item.catalog_name}</span>
                </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-mm-txs capitalize">{order.method || 'N/A'}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 bg-mm-gbg/30 border-t border-mm-crd flex items-center justify-between">
        ...
        <div className="flex gap-3">
          ...
          {order.status === 'delivered' && onRate && (
            <Button size="sm" onClick={() => order.store_id && onRate(order.store_id)}>Calificar</Button>
          )}
          {order.status === 'pending' && <Button variant="danger" size="sm">Cancelar</Button>}
        </div>
      </div>
    </motion.div>
  );
