...
import { Plus, Edit2, Trash2, Search, X, FileCheck } from 'lucide-react';
...
export function DocumentTypesTab() {
  ...
  } = useTable({ initialData: filteredTypes });
  ...
  const handleOpenEdit = (item: DocumentTypeRow) => {
    setEditingType(item);
    setFormData({
      name: item.name,
      slug: item.slug,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(null);
      await saveDocumentType(editingType ? editingType.id : null, {
        name: formData.name,
        slug: formData.slug,
  ...
  const columns = [
      ...
      render: (item: DocumentTypeRow) => (
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-mm-gbg rounded-lg flex items-center justify-center text-mm-g font-bold shrink-0">
            <FileCheck className="w-5 h-5 text-mm-g" />
          </div>
          <div>
    ...
    },
    {
      ...
      sortable: true,
      render: (item: DocumentTypeRow) => (
        <Badge variant={item.is_required ? 'error' : 'default'}>
  ...
  if (loading) return <div className="p-8 text-center text-mm-txs">Cargando tipos de documento...</div>;
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Button size="sm" onClick={handleOpenAdd}>
          <Plus className="w-4 h-4 mr-2" /> Nuevo Tipo de Documento
        </Button>
      </div>
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <Input
                  ...
                  placeholder="Ej: Registro Único Tributario (RUT)"
                  required
                />

                <Input
                  label="Slug o identificador"
                  value={formData.slug}
                ...
                <label className="flex items-center gap-3 cursor-pointer py-2">
                  ...
                  />
                  <span className="text-sm font-bold text-mm-g">Requerido para verificación de tienda</span>
                </label>
