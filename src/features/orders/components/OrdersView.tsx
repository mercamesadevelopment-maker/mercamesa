...
export function OrdersView({
  ...
  const handleConfirmStatusChange = async (notes: string) => {
    if (statusChangeRequest) {
      ...
      // Actualizar la orden seleccionada si el modal de detalle está abierto
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-mm-txs">
            {variant === 'admin'
              ? 'Gestiona todos los pedidos de la plataforma en tiempo real.'
              : 'Gestiona el flujo de trabajo de tu tienda en tiempo real.'}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {loading ? (
          <div className="col-span-full py-20 text-center">
            <Loader2 className="w-10 h-10 animate-spin mx-auto mb-4 text-mm-g" />
            <p className="text-sm font-semibold text-mm-txs">Cargando pedidos de la plataforma...</p>
          </div>
        ) : (
