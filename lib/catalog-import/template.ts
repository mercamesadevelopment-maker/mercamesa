...
export async function buildCatalogTemplate(input: CatalogTemplateInput): Promise<Buffer> {
  ...
  const ranges = buildListsSheet(workbook, {
  ...
  buildInstructionsSheet(workbook, 'Carga masiva del catálogo maestro', [
    ...
    '7. Si el nombre ya existe en el catálogo, esa fila se omite y te decimos en qué categoría está el existente.',
    '8. Los productos se crean SIN imagen y activos. La foto se agrega después editando cada producto.',
    `9. Puedes cargar hasta ${MAX_CATALOG_ROWS} productos por archivo. La plantilla trae ${TEMPLATE_BLANK_ROWS} filas con desplegable; si necesitas más, copia el formato hacia abajo.`,
    '10. Guarda como Excel (.xlsx) o CSV y súbelo desde Catálogo Maestro → Carga masiva.',
  ]);
